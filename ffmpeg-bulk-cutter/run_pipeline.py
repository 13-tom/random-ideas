...
def main():
    ...
    parser.add_argument("--track-mode", choices=["dynamic", "static", "fanpage"], default="dynamic", help="dynamic = pan to follow the subject (default). static = one fixed, face-informed crop position for the whole clip - no panning, no possible camera shake. fanpage = tighter zoom on a lone subject, slower/steadier pan, and automatically eases out to a wider crop that fits everyone when a 2nd person enters frame. Only relevant with --track-faces.")
    ...
    if args.aspect != "original":
        ...
        for i, clip_path in enumerate(clip_paths, start=1):
            ...
            suffix = f" (tracking faces, {args.track_mode})" if args.track_faces else ""
            print(f"[{i}/{len(clip_paths)}] {clip_path.name} -> {args.aspect}{suffix}  =>  {output_path}")
            if args.track_faces and args.track_mode == "fanpage":
                fanpage_crop.fanpage_track_and_crop(clip_path, output_path, args.aspect, target_res, use_gpu_track, reframe.reframe_video, args.gpu_detect)
            elif args.track_faces:
