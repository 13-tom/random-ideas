...
## GUI (no terminal commands to remember)

Prefer clicking over typing CLI flags? `gui/app.py` is a desktop app that
wraps `run_pipeline.py` behind a window: pick your video, add clip start/end
times in a table (or load a CSV), tick the options you want, and hit **Run
pipeline** — output and progress stream into a log panel in the same window.

Requires [ffmpeg](#setup) installed first, same as the CLI. Then:

- **Mac/Linux:** double-click `run_gui.sh` (or run `./run_gui.sh` in a terminal)
- **Windows:** double-click `run_gui.bat`

First run creates a local `.venv` and installs everything automatically —
this can take several minutes (it pulls the same captioning dependencies as
the CLI, including `torch`, so a slow connection means a longer wait). Every
run after that opens instantly. See `gui/requirements-gui.txt` for the one
extra GUI-only dependency (`customtkinter`).
